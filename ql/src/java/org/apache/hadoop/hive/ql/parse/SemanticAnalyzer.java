...
public class SemanticAnalyzer extends BaseSemanticAnalyzer {
  ...
  /*
   * Capture the CTE definitions（Common Table Expression） in a Query.
   */
  /*
  ...
  private void doPhase1GetAllAggregations(ASTNode expressionTree,
    ...
    if (exprTokenType == HiveParser.TOK_FUNCTION
      ...
      //具体函数比如count的type 是HiveParser.Identifier
  ...
  int[] findTabRefIdxs(ASTNode tabref) {
    ...
    int tsampleIndex = -1;
    int ssampleIndex = -1;
    //从第一个开始而不是从第0个，是因为第0个是真正的索引
    //(tok_from (tok_tabref (tok_tabname words) a))
  ...
  private String processTable(QB qb, ASTNode tabref) throws SemanticException {
    ...
    //拿到alias(别名),TOK_TABLEBUCKETSAMPLE, TOK_TABLESPLITSAMPLE,TOK_TABLEPROPERTIES在tok_tabref子节点的位置(index)
    //比如 (tok_from (tok_tabref (tok_tabname words) a))
    //a 就对应的别名
  ...
  private void processJoin(QB qb, ASTNode join) throws SemanticException {
    ...
    //记录增加join数(noOfJoin, noOfOuterJoin)
  ...
  public boolean doPhase1(ASTNode ast, QB qb, Phase1Ctx ctx_1, PlannerContext plannerCtx)
    ...
    /*
    *终结节点
    *TOK_SELECTDI,TOK_SELECT,TOK_WHERE,TOK_INSERT_INTO,TOK_DESTINATION,TOK_FROM,TOK_CLUSTERBY,TOK_DISTRIBUTEBY,TOK_SORTBY,TOK_ORDERBY,TOK_GROUPBY,
    *TOK_ROLLUP_GROUPBY,TOK_CUBE_GROUPBY
    *TOK_GROUPING_SETS,TOK_HAVING,KW_WINDOW,TOK_LIMIT,TOK_ANALYZE,TOK_UNIONALL,TOK_INSERT,TOK_LATERAL_VIEW,TOK_LATERAL_VIEW_OUTER
    *TOK_CTE
     */
    /*
    select count(*) from words
    (tok_query (tok_from (tok_tabref (tok_tabname words))) (tok_insert (tok_destination (tok_dir tok_tmp_file)) (tok_select (tok_selexpr (tok_functionstar count)))))
     */
...
//        /(tok_destination (tok_dir tok_tmp_file))
      ...
      case HiveParser.TOK_FROM:
        ...
        //查看TOK_FROM下的子节点 是否有子查询
        /*
        *TOK_TABREF(表明),TOK_VIRTUAL_TABLE,TOK_LATERAL_VIEW,TOK_LATERAL_VIEW_OUTER
        * TOK_LATERAL_VIEW_OUTER
        * TOK_JOIN,TOK_CROSSJOIN,TOK_LEFTSEMIJOIN,TOK_UNIQUEJOIN
        * TOK_PTBLFUNCTION
         */
  ...
  private void getMetaData(QB qb, ReadEntity parentInput)
    ...
    for (String alias : tabAliases) {
     ...
     if (tab.isView()) {
      ...
      //记录起别名和实际的table关系，该tab包含了该表的元数据信息，org.apache.hadoop.hive.metastore.api.Table
  ...
  * */
  ...
  private Operator genGroupByPlanMapGroupByOperator(QB qb,
    ...
    RowResolver groupByInputRowResolver = opParseCtx.get(inputOperatorInfo)
        .getRowResolver();
    System.out.printf("edwin genGroupByPlanMapGroupByOperator groupByInputRowResolver:%s \n", groupByInputRowResolver);
    ...
    op.setColumnExprMap(colExprMap);
    System.out.printf("edwin genGroupByPlanMapGroupByOperator operator:%s \n", op.dump(0));
    return op;
  }
  ...
  private Operator genGroupByPlanMapAggr2MR(String dest, QB qb,
    ...
    List<ASTNode> grpByExprs = grpByExprsGroupingSets.getFirst();
    List<Integer> groupingSets = grpByExprsGroupingSets.getSecond();
      System.out.printf("edwin genGroupByPlanMapAggr2MR grpByExprs:%s\n", grpByExprs);
      System.out.printf("edwin genGroupByPlanMapAggr2MR groupingSets:%s\n", groupingSets);
    boolean groupingSetsPresent = !groupingSets.isEmpty();
  ...
  private Operator genJoinReduceSinkChild(QB qb, ExprNodeDesc[] joinKeys,
    ...
    for (int i = 0; i < columns.size(); i++) {
      ...
      System.out.printf("edwin genJoinReduceSinkChild nm:%s, nm2:%s, expr:%s \n",
              nm != null?nm.toString():"null", nm2 != null?nm2.toString():"null", expr);
    ...
    if (reduceKeys.size() == 0) {
      numReds = 1;
      //set hive.strict.checks.cartesian.product =true return false NO_CARTESIAN_MSG
      //set hive.strict.checks.cartesian.product =false return true null
  ...
  private Operator genTablePlan(String alias, QB qb) throws SemanticException {
    ...
    System.out.printf("edwin genTablePlan TableScanOperator:%s, properties:%s \n",
            ((top == null)?"null":top.toString()), (properties == null)?"null":properties.toString());
    if (top == null) {
...
//    }
 ...
 System.out.println("edwin OUTER QB aliases:");
    ...
    * 执行计划肯定是从tablescan开始的,topOps记录这些
    * */
    copyInfoToQueryProperties(queryProperties);
    ...
    System.out.printf("edwin Generate table access stats:%b \n",
            HiveConf.getBoolVar(this.conf, HiveConf.ConfVars.HIVE_STATS_COLLECT_TABLEKEYS));
...
* */
  ...
  public void processPositionAlias(ASTNode ast) throws SemanticException {
    ...
    while (!stack.isEmpty()) {
      ...
      // get node type
      //逐层遍历节点 查看是否有 TOK_SELECT， TOK_GROUPBY， TOK_ORDERBY
